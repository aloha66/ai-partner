...
- PR #4 internal beta packaged smoke passed on 2026-06-18 from `main@42b742e` (`42b742e fix: follow up interactive workflow card semantics`). The refreshed DMG hash is `e6e27c086916f7e0e3238afe1871b062ee9118597996e06668d046686af9d23e`; packaged runtime descriptor permissions stayed `0700`/`0600`; `debug:discover` found `http://127.0.0.1:58481/events`; `debug:send waiting` with authorization metadata (`authorization.id=auth_pr4_internal_beta`, `status=pending`) and `debug:send done --run-id run_smoke_20260618T002008Z` both succeeded against the same active run. Focus checks stayed on `Codex`, and the saved descriptor copy was rejected as `descriptor_stale` after process exit.
